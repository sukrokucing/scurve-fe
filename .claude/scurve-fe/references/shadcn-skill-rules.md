...
1. **Tailwind v4** — CSS-first config via `@tailwindcss/postcss`. No `tailwind.config.js`. Theme tokens defined in CSS with `@theme`. All utility classes still work.

2. Current project uses established wrappers (`Form`, `Input`, `Combobox`, etc.).
- Apply these rules incrementally; do not force broad rewrites that break contracts.

3. Keep existing UX/test contracts stable.
- Preserve `data-testid` selectors and existing interaction behavior unless explicitly changing them.

4. Use these rules as a review checklist.
